...
public class DatabaseSeeder
{
    ...
    private static void CreateSaleProduct(InventoryDbContext context)
    {
        if (context.SalesProducts.Any()) return;

        var products = context.Products.Select(p => p.Id).ToArray();
        var sales = context.Sales.Select(s => s.Id).ToArray();
        var list = new List<SaleProduct>();

        for (int i = 0; i < 50; i++) 
        {
            var saleProduct = SaleProductCreator.SaleProductCreate(products,sales);
            list.Add(saleProduct);
        }
        context.SalesProducts.AddRange(list);
        context.SaveChanges();
    }
